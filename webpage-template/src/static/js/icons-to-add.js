...
// Format is set to pull location, name, and habitat from an array. It may need to be adjusted for our data source
    ...
    //Puts a custom flower icon at each endangered species location and adds a popup of the species name and proected habitat (NWR)
    for (let i = 0; i < plantData.length; i++) {
        let plantMarker = plantData[i];
        L.marker(plantMarker.location, {icon: flowerIcon})
        .bindPopup("<h2>Species: " + plantMarker.name + "</h2> <hr> <h3>Protected Habitat: <br>" + plantMarker.habitat + "</h3>")
        .addTo(myMap);
    }